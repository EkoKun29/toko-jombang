...
@section('content')
    <div class="card-body">
        ...
        @include('components.button-back', [
            'label' => 'Back',
            'url' => '/report-penjualan-piutang',
        ])
        <form action="{{ route('penjualan-piutang.update-induk', $head->id) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="row">
                <div class="col-4">
        ...
        <table class="table table-bordered table-hover bg-white">
            ...
            <tbody>
                @foreach ($data as $item)
                    <form action="{{ route('report-penjualan-piutang.update', $item->id) }}" method="post">
                        @csrf
                        <tr>
